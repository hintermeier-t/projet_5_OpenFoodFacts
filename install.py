...
class Cleaner:
    ...
    save in the DB only the products with all the required informations. Then,
    it will format some information (like Category, Store Name etc) to avoid
...
class Saver:
    ...
    def __init__(self, data_list):
        ...
        for data in data_list:
            ...
            self.categories.extend([cat.strip()
                for cat in data.get('categories').split(',')])
            self.stores.extend([store.strip()
                for store in data.get('stores').split(',')])
    ...
    def associate(self, data_list):
        """The associate method create both Categorized and Buyable association
        tables.
